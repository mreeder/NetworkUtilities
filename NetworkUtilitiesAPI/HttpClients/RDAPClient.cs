...
namespace NetworkUtilitiesAPI.HttpClients
{
    public class RDAPClient : IRDAPClient
    {
        ...
        public RDAPClient(HttpClient client, IConfiguration config, ILogger<RDAPClient> logger)
        {
            _logger = logger;
            _client = client;
            _client.BaseAddress = new Uri(config["ApiEndpoints:RDAP"]);    //TODO: Handle missing/misconfiguration exceptions
